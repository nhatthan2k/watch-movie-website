...
export const GET_ALL_PRODUCT = createAsyncThunk('product/GET_ALL_PRODUCT', async ({ search, category, page }) => {
    let response = await instance.get(`/api/products/?category=${category}&page=${page}&search=${search}`);
...
export const POST_ADD_PRODUCT_DETAIL = async ({ formProductDetail, idProduct }) => {
    let resp = await instance.post(`/api/products/${idProduct}`, formProductDetail, {
        headers: { Authorization: `Bearer ${new Cookies().get('token')}` },
    });
    return resp;
};

export const PUT_UPDATE_PRODUCT_DETAIL = async ({ formProductDetail, idProductDetail, idProduct }) => {
    let resp = await instance.put(`/api/products/${idProductDetail}/in/${idProduct}`, formProductDetail, {
        headers: { Authorization: `Bearer ${new Cookies().get('token')}` },
    });
    return resp;
};

export const PUT_STATUS_PRODUCT_DETAIL = async ({ idProductDetail, idProduct }) => {
    let resp = await instance.put(
